...
const registerUser = async (req, res) => {
    ...
    if(!req.body) {
        throw new ApiError(400, {message: 'req.body is not sent'}, 'Body is missing')
    }

    const {fullName, username, email, password} = req.body

    if(!(fullName && username && email && password)) {
        throw new ApiError(400, {message: 'Any field is missing'}, 'Some field is missing')
    }

    if(!(email.includes('@'))) {
        throw new ApiError(400, {message: 'Email is not valid'}, 'Provided email is not valid')
    }
    ...
    if(!user) {
        throw new ApiError(400, 'Unable to create the user', 'Unable to create the User')
    }
    ...
    res.status(200).json( new ApiResponse(200, user, 'User is successfully registered'))
...
const verifyOtp = async(req, res) => {
    ...
    res.status(200).json(new ApiResponse(200, user, 'User is verified succuessfully'))
...
const forgotPassword = async (req, res) => {
    ...
    res.status(200).json(new ApiResponse(200, user, 'Otp is sent please enter that '))
...
const newPassword = async (req, res) => {
    ...
    res.status(200).json( new ApiResponse(200, user, 'Password has changed successfully'))
}
